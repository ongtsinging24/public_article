...
> ---
>
> ### 📌 2026-08-31 追記：已完成 out-of-sample 複核 → [[31-08-2026_CC_FP-p9極端greek的OOS複核]]
>
> 用本篇截止日**之後**的資料（session 2026-07-24 → 08-28）獨立重跑：
> - ✅ **結論 2（cohort B「真買 call」是反指標）OOS 複製成功**：OOS n=10、5td mean **−3.58%**、P(>0) **30.0%**
>   （剝 regime 後 −4.55%，而同期 BASELINE +0.56%、SPY +1.00% ⇒ **是逆勢不是隨大盤**）。
>   POOLED n=47、cluster bootstrap CI [15.6, 40.9] 與 BASELINE [46.9, 54.0] **仍不重疊**。
> - ✅ **結論 1（cohort A 無 edge）維持**：POOLED n=23、P(>0) 52.2% [33.0, 70.8]，把 BASELINE 50.7% 包住。
> - ⚠️ **§4.3 第一項待辦（`alpha.py` 加 gamma 條件）已作廢**：ROMA v3.66（2026-07-31）
>   直接把四個方向軸**全數拔除**、`fp_score ≡ 0`，比本篇的建議更徹底（現況見 `alpha.py:120-135`、`:271-277`）。
> - 🔴 **本篇不可複製**：附錄列的 `py_dir/fp_trend/exp_sellside_triplet.py` **已不存在於工作區**，
>   新篇是照 §2 口徑獨立重寫，因此數字有 1–2pp 漂移（BASELINE 51.3%→49.1%、A n=17→20、B n=36→37）。
>   **定性結論全部一致，但量級請以新篇為準。教訓：實驗腳本必須進版控。**
...
> 🔴 **2026-08-19 再更新**（[[19-08-2026_CC_聚合層級先於保守化_列權重假象]]，樣本擴至 83 場次 / 15,810 列）：
> 上面第 1 點「買 call ≠ 機構偏多、應寫**短期過熱**」的**推廣部分作廢** ——
> C/P 軸那 9.7pt 整條是**列權重假象**，事件層去重（sym×date×cp）後 call 48.4% / put 51.0%，
> 兩側 CI 都蓋住對照 49.0%；面板迴歸加 date FE＋過去報酬＋標的 FE 後 CALL +0.029pp [−0.227, +0.286]。
> ⇒ 「短期過熱」這個讀法沒有實證依據，ROMA v4.24 已把標籤改為純計數（`call_side_heavy`）。
> ✅ **但本篇自身的方法沒有中招**：§觀測 已做「每 sym 只取首次」(n=33) 與 cluster bootstrap，
>    正是新篇建議的事件層口徑 —— 本篇的**保守慣例反而是對的示範**，只有引用他篇的推論那段要撤。
> ⚠️ 惟本篇 n=658 列 / 133 sym 的**列層級數字**仍應照新篇 §一 重讀（列層級答的是「隨機抽一個合約」）。
