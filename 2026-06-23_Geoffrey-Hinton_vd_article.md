...
### 14.3　引擎拆解：五個核心零件（把縮寫講透）
把上面那台引擎拆開，順著「**被訓練的對象 → 怎麼算梯度 → 怎麼用梯度 → 怎麼計價**」一條線走，正好對應開頭那組縮寫：

**① MLP（多層感知器）——被訓練的「對象」**
一個 MLP 就是「**線性變換 + 非線性**」逐層堆疊：每層做一次矩陣乘法（權重 `W`）＋偏置，再過一個激活函數（如 ReLU）。所謂「學習」，本質就是調這些 `W`。名字裡的「感知器」正是下一節（§14.4）會提到、1969 年被 Minsky–Papert 判死刑的單層 perceptron——**多層化 + 反向傳播，才讓它起死回生**。一句話：MLP 是旋鈕的容器，BP 是轉旋鈕的手。

**② autodiff（自動微分）——BP 的「真身」**
這是最常被誤解的一層：**反向傳播其實是「反向模式自動微分（reverse-mode autodiff）」套用在神經網路上的特例**。三者要分清——
- **鏈式法則**是數學；**BP** 是把鏈式法則在「計算圖」上高效執行的演算法；**autodiff 引擎**是把這件事自動化的軟體。
- autodiff 有兩個方向：**前向模式**成本正比於「輸入數」，**反向模式**成本正比於「輸出數」。神經網路恰好是**上億個輸入參數、卻只有一個純量損失**——所以反向模式（一次回掃就拿到全部梯度）完勝，這正是 BP「便宜」的數學根源。
- 今天 PyTorch / TensorFlow / JAX 的核心都是一具 autodiff 引擎：你只寫前向、框架自動建**計算圖**、一行 `.backward()` 就回傳所有梯度——工程師再也不必手推導數。**BP 從一篇 1986 年的論文，濃縮成一行 `loss.backward()`。**

**③ GD / SGD（梯度下降）——拿梯度去「做什麼」**
BP 算出「往哪調」，GD 負責「真的去調」。把損失想成一座多維山谷，梯度是最陡下坡方向，每步走「學習率（learning rate）」那麼遠：

| 變體 | 每步用多少資料 | 取捨 |
|------------------|------------------|----------------------------------|
| 全批次 GD | 全部資料 | 方向最準，但又慢又吃記憶體 |
| SGD | 1 筆 | 快、但噪聲大 |
| **mini-batch**（實務主流） | 一小批（如 256） | 速度/穩定的甜蜜點；那點噪聲反而幫忙跳出鞍點 |

實務上再疊一層**優化器**——動量（momentum）→ RMSProp → **Adam**（近乎預設）——用慣性與自適應步長加速收斂。重點：**SGD 的「隨機」不是缺點，那點噪聲正是深網路不卡死在壞極值的關鍵。**

**④ FLOPs（浮點運算）——把上述一切換算成「錢與電」**
最後一步是把訓練成本量化。一次前向傳播約需 `2N` 次浮點運算（`N`＝參數量／每個 token），反向傳播約是前向的**兩倍**，於是有那條業界常用的估算式：

> **訓練總算力 C ≈ 6 × N（參數量）× D（訓練 token 數）**

這條看似樸素的公式，是連接「演算法」與「資本支出」的**匯率**：GPT 級訓練落在 **10²⁴–10²⁵ FLOPs** 量級；Chinchilla 縮放律更指出 `N` 與 `D` 要同步放大（compute-optimal 約 `D ≈ 20N`）。**參數一漲，FLOPs 隨之倍增，直接翻譯成 GPU 卡數、機房電力與資本支出**——這正是文末投資視角 box 的量化底座。

> 把這四個零件串起來一句話：**MLP 提供旋鈕，autodiff/BP 算出每個旋鈕該怎麼轉，GD/SGD 真的去轉，FLOPs 則是這一切轉動所燒掉的算力與電。**

### 14.4　辛頓的真實角色：不是發明者，而是「教父」
...
> **投資視角**：反向傳播是 AI 產業看不見、卻無所不在的**數學引擎**——每一次模型訓練，本質都是 BP 在億萬參數上的反覆求導，而「便宜到能擴展」的特性，直接把模型規模換算成 **FLOPs → GPU 算力 → 電力**的線性需求。理解這點，就理解了為何 §6–§7 的「需求證明鏈」是**結構性**的：只要產業仍用 BP 訓練（短中期內無可替代），算力需求就與模型規模同步膨脹。值得長線留一隻眼的尾部風險是 §14.7——若辛頓這類「非 BP / 生物式」學習範式哪天成熟，可能改寫「訓練＝海量 GPU」的成本結構（目前仍屬研究早期，非可交易事件，但屬於會顛覆 NVDA 敘事根部的少數技術變數之一）。
